...
class OutlineGenerationAgent:
    ...
    def execute(self, agent_input):
        ...
        try:
            bundle = validate_outline_dependencies(agent_input)
            ctx = build_outline_context(bundle, agent_input)
            valid = set(ctx["valid_source_filenames"])

            tm = dict(
                zip(
                    bundle["kb"].source_filename.astype(str),
                    bundle["kb"].title.astype(str)
                )
            )

            title_to_source = {v: k for k, v in tm.items()}

            out = Path(agent_input.agent_context.output_directory)
            manifest_path = out / "outline_generation_manifest.json"
            ...
            if manifest_path.exists() and not agent_input.policy["force_rebuild"]:
                try:
                    ...
                    # Nunca reutilizar una salida previa cuyo validation_report
                    # no haya sido aprobado. El fingerprint solo demuestra que
                    # las entradas upstream no cambiaron, no que el outline sea válido.
                    reuse = (
            ...
            if reuse:
                outline = json.loads((out / "state_of_art_outline.json").read_text())
                raw = (out / "state_of_art_outline_raw.txt").read_text()
            ...
            sr, us = repair_outline_sources(
                ...
                tm,
                title_to_source,
                float(agent_input.policy.get("title_match_cutoff", 0.55))
            )
            ...
            # Repara secciones que quedaron sin papers usando únicamente
            # los temas y papers representativos producidos por el agente 04.
            # Si no existe evidencia upstream suficiente, no fuerza la reparación.
            themes = (
            ...
            artifacts = write_outline_artifacts(
                out,
                outline,
                raw,
                validation,
