...
def check_no_claim_uid_collisions(records: list[ClaimIdentityRecord]) -> None:
    """Invariante 1 (unicidad): ningún ``claim_uid`` se reutiliza para
    dos claims ACTIVOS distintos en el mismo conjunto resuelto (una
    sección, o el draft completo). Falla cerrado con
    ``CLAIM_IDENTITY_UID_COLLISION`` si encuentra una colisión --
    nunca renombra ni descarta en silencio."""

    seen: dict[str, ClaimIdentityRecord] = {}
    for record in records:
        if record.claim_uid in seen:
            raise ValueError(
                f"CLAIM_IDENTITY_UID_COLLISION:{record.claim_uid} "
                f"(claim_id={seen[record.claim_uid].claim_id!r} y claim_id={record.claim_id!r} "
                "son claims activos distintos con el mismo claim_uid)"
            )
        seen[record.claim_uid] = record


def enforce_forced_claim_uid_continuations(
    *, resolved_claims: list[ClaimIdentityRecord], forced_parent_uids: tuple[str, ...],
) -> None:
    """Invariante del requisito 1, a nivel de SECCIÓN (no de un único
    claim): para cada ``claim_uid`` que un issue de ``writer_revision_
    request`` señaló explícitamente para ESTA sección, debe existir
    entre ``resolved_claims`` un claim cuyo ``parent_claim_uids ==
    (ese_uid,)`` -- es decir, que efectivamente lo continuó.

    Se verifica DESPUÉS de resolver todos los claims de la sección
    (nunca se sabe de antemano cuál claim de salida corresponde a cuál
    issue -- eso lo declara el LLM). Si un ``claim_uid`` forzado no
    aparece continuado por ningún claim resuelto (porque el LLM lo
    declaró ``NEW``, lo fusionó, o simplemente no lo mencionó), falla
    cerrado -- nunca se asume ni se sobrescribe en silencio."""

    continued_parents = {
        record.parent_claim_uids[0]
        for record in resolved_claims
        if len(record.parent_claim_uids) == 1
    }
    for forced_uid in forced_parent_uids:
        if forced_uid not in continued_parents:
            raise ValueError(
                f"CLAIM_IDENTITY_FORCED_UID_MISMATCH: writer_revision_request señalaba "
                f"claim_uid={forced_uid!r} para esta sección, pero ningún claim de la "
                "respuesta lo continuó (action=CONTINUE con ese claim_uid como único padre)."
            )
